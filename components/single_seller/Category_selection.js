...
import { menu } from "../../data/seller_menu"
...
const Category_Selection=()=>{
    ...
    return(
        ...
        {/* category section  */}
        <div className="md:w-[100%] lg:w-[75%] xl:w-[60%]   flex flex-col ml-auto justify-between items-center md:flex-row md:justify-end relative">
          ...
          {menu.map((item, index) => {
               return (
                <div className={router.asPath.includes(`${item.path}`)?"mini_bar_item active":"mini_bar_item"} key={index} >
                 <Link  href={`/sellers/${id}/${item.path}`}><a className="md:block flex justify-between items-center md:py-0 md:px-0" onClick={()=> setMenuOpen(false)}>
