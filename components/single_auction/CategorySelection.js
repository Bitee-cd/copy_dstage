...
import { menu } from "../../data/auction_menu"
import { useState } from "react"

const Category_Selection=()=>{
    const router =useRouter()
    const id = router.query.auctionId
    ...
    return(
        <div className="hidden md:block w-[90%] mx-auto max-w-[1440px]">
        {/* category section  */}
          ...
          {menu.map((item, index) => {
               return (
                <div className={router.asPath.includes(`${item.path}`)?"mini_bar_item active":"mini_bar_item"} key={index}>
                 <Link  href={`/auctions/${id}/${item.path}`}><a className="md:block flex justify-between items-center pr-3  md:py-0 md:px-0" onClick={()=> setMenuOpen(false)}>
